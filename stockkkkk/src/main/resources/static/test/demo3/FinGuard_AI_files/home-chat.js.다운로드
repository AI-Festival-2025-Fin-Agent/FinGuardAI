class HomeChat {
    ...
    bindEvents() {
        ...
        // 모바일 키보드 대응: 입력창 포커스 시 스크롤
        this.chatInput.addEventListener('focus', () => {
            this.scrollInputIntoView();
        });

        // iOS Safari 키보드 이슈 대응
        if (this.isIOS()) {
            this.chatInput.addEventListener('touchstart', () => {
                setTimeout(() => {
                    this.scrollInputIntoView();
                }, 300);
            });
    ...
    async handleAnalysisClick(mode, query, summary, messageDiv) {
        ...
        try {
            ...
            if (analysis) {
                ...
                analysisDiv.innerHTML = renderedAnalysis;
